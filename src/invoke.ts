...
export type InvokeEvent =
   ...
   * the `html` parse kind in argv.ts for why this exists.
   */
 ...
 *   - pass `signal` to abort → the child is killed with SIGTERM and the
 *     stream closes without a `done` event
 *   - cancel the stream → the child is killed as well
 *
 * Errors (unknown agent, missing binary, override typo, spawn failure) are
 * surfaced as `{ type: "error" }` events followed by close — never thrown.
 */
